class LineChart {
	constructor(opts) {
		...
		this.HEIGHT =
			window.innerHeight / 2.2 - this.MARGIN.TOP - this.MARGIN.BOTTOM;
	...
	draw(currMetric) {
		...
		const x = d3
			.scaleLinear()
			.domain([
				d3.min(currData, (d) => d[this.xValue]) - 1,
		...
		const y = d3
			.scaleLinear()
			.domain([0, d3.max(currData, (d) => +d[this.yValue]) + 3])
			.range([this.HEIGHT, 0]);

		this.lineActual = d3
			.line()
			.x(function (d) {
				return x(d[that.xValue]);
			})
			.y(function (d) {
				return y(+d[that.yValue]);
			})
		...
		// add line chart
		let line = this.svg.selectAll("path.met").data([currData]);

		let lineEnter = line.enter().append("path").attr("class", "met");
		...
		circles
			.on("mouseover", function (d) {
				// show it and update html
				let pointData = d;
				d3.selectAll(`circle.${this.yValue}`).style("opacity", 0);
				d3.selectAll(`circle.cnt`).style("opacity", 0.2);
				d3.selectAll("path.met").style("opacity", 0.2);
				d3.select(this).style("opacity", 0.7);
				that.tooltip.transition().style("opacity", 0.9);
				that.tooltip
					.html(function (d) {
			...
			.on("mouseout", function (d) {
				d3.selectAll(`circle.cnt`).style("opacity", 1);
				d3.selectAll("path.met").style("opacity", 1);
				that.tooltip.transition().style("opacity", 0);
			});
	}
}
